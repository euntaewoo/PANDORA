...
이 문서는 루트 경로에 구축된 `multilingual_text_in_image_translation.py` 엔진 시스템을 제어하는 최상위 시스템 룰 파일입니다. 에이전트는 다국어 이미지 번역 엔진을 다룰 때 이 문서의 규칙을 절대적으로 준수해야 합니다.

## 1. 아키텍처 원칙: 설정 주도형 플러그인 격리 (Configuration-Driven Pack)
- **하드코딩 금지**: 코어 엔진 스크립트(`multilingual_text_in_image_translation.py`) 내에 특정 언어의 폰트 명칭, 규제 금지어, 약기법 등을 `if/else`로 하드코딩하는 것을 절대 금지합니다.
...
- **Architecture Pattern**: 단일 통합 코어(`multilingual_text_in_image_translation.py`) 기반 언어별 분기 및 고시표 HTML 자동 렌더링.
