...
# verify_pipeline.py 동적 상대경로 리팩토링
...
def test_pipeline():
    ...
    for lang in ["EN", "JP", "CN", "TW"]:
        lex_data = load_dynamic_compliance_lexicon(lang)
        prompt_txt = lex_data.get("prompt_text", "")
        rules_count = len(lex_data.get("banned_patterns", {}))
        print(f"[{lang}] Loaded {rules_count} active replacement rules. Prompt length: {len(prompt_txt)} chars.")
    
    print("\\n=== [TEST 2] Deterministic Override Gate 5대 문구 차단 검증 ===")
    mock_items = [
        {"original": "Solution for Complex skin issues and troubled skin"},
        {"original": "Provides nutrients for cellular vitality and combats premature aging"},
        {"original": "Reinforces cellular resilience for firm skin"},
        {"original": "Prescribe multivitamin complex to infused daily"},
        {"original": "Boosts Bio-Immunity for Kyel-Tan-Tone"}
    ]
    
    res = apply_deterministic_qa_overrides(mock_items, "EN")
    ...
    assert "reinforces skin's natural moisture barrier" in res[2].get('translated', res[2].get('original').lower())
...
print("SUCCESS: Refactored verify_pipeline.py to dynamic relative paths.")