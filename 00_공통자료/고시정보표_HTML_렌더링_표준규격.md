...
1. **1열 라벨(th) 최적 너비 및 텍스트 정렬**:
   - **중화권 (중국어 간체 CN / 번체 TW·HK)**: `width: 250px; padding: 20px 15px;` (좌우 여백 30px, 2열 순수 본문 폭 570px 극대화 황금비율)
   - **한국어 (KO)**: `width: 295px; padding: 24px 20px; cell_size: 30px;` (Pretendard 30px Bold, 긴 라벨 2줄 황금분할)
   - **영어 (EN)**: `width: 295px; padding: 10px 12px; cell_size: 28px; line-height: 1.36; letter-spacing: -0.8px;` (Pretendard 28px Bold, 순수 가용폭 확보로 `Country of Origin`, `Precautions for Use`, `Customer Service` 100% 1줄 안착 및 `Quality Assurance Standard` 2줄 대칭 안착, **세로 2570px 1장 수납 보장**)
   - **일본어 (JP)**: `width: 280px; padding: 24px 20px;` (Noto Sans JP 32px 기준)
   - **정렬 및 줄바꿈 기준**: `text-align: left; vertical-align: middle; word-break: keep-all; overflow-wrap: break-word;` (영문/다국어 단어 중간 분절 결함 100% 원천 차단)
...
2. **2열 본문(td) 폭 확장 및 단어 결속 보호**:
   ...
   - 주의사항 1번 문단 등 장문 텍스트가 억지 개행 없이 자연스럽게 안착.
   - 전문의 상담 어휘(`专业医生`, `专业医师`, `專業醫師`, `전문의 등과 상담할 것`) 등 문장 끝 핵심 구문은 `<span style="white-space: nowrap">` 또는 `\u00A0`로 묶어 외톨이 글자(Orphan) 원천 차단.
3. **법인명 약칭 `(주)` 보존**:
...
5. **고시정보표 전담 프론트엔드 QA 서브에이전트 연동 및 시각 디코딩 의무화**:
   - 렌더링 직후 `notice_table_frontend_qa_agent`가 PNG 산출물을 `view_file` 도구로 무조건 자동 디코딩하여 1열 라벨 규격, 10자 미만 1줄 유지, 단어 쪼개짐(`Or/igin`, `St and/ard`), 외톨이 글자 유무를 픽셀 단위로 자가 검수하고 결함 발견 시 즉시 자가치유 루프 실행.
...
| :--- | :---: | :---: | :---: |
| **영문(EN)** | **`52px`** (Bold 700) | **`28px`** (Bold 700) | **`28px`** (Regular 400) |
| **일본어(JP)** | **`60~64px`** (Bold 700) | **`30~32px`** (Bold 700) | **`30~32px`** (Regular 400) |
