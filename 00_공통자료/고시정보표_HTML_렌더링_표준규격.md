...
1. **1열 라벨(th) 최적 너비 및 텍스트 정렬**:
   - **중화권 (중국어 간체 CN / 번체 TW·HK)**: `width: 270px; padding: 20px 15px;` (좌우 여백 30px, 2열 순수 본문 폭 520px 극대화 황금비율)
   ...
   - **단문 라벨 1줄 유지 원칙**: 10자 미만 라벨(`使用注意事项` 6자, `消费者咨询电话` 7자, `使用方法` 4자, `注意事項` 4자)은 절대 2줄로 쪼개지 않고 **100% 1줄 유지**.
   - **10자 이상 긴 라벨 분할**: `特殊用途化妆品<br>审查状态`, `特殊用途化粧品<br>審查狀態`, `化妆品生产企业 /<br>责任销售商` 등 의미 단위 2줄 황금 분할.
2. **2열 본문(td) 폭 확장 및 단어 결속 보호**:
   - 중화권(간체/번체) 기준 2열 순수 본문 폭을 **`520px`**로 확장 (`val_padding: 20px 15px`, `letter-spacing: 0px`).
