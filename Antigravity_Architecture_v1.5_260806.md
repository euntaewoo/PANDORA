...
│
├── Projects\ (또는 Desktop\multilingual_text_in_image_translatio_agy_sdk\)      ← [로컬 실무 작업 공장 (Local Workspace)]
│     ├── 01_번역대상_원본\                           ← ★ [통합 공통 인풋] 한국어 원본 상세페이지/제품 이미지 수납
│     ├── 02_번역결과_최종\                           ← ★ [통합 공통 아웃풋] [상품명]_[언어명] 자동 격리 저장
│     ├── multilingual_text_in_image_translatio_agy_sdk_core\    ← ★ [공식 4대 다국어 통합 원클릭 엔진 본체]
│     │     ├── multilingual_text_in_image_translatio_agy_sdk_core.py
│     │     ├── 다국어_통합번역_원클릭실행.bat
│     │     ├── 실행_방법_가이드.md
│     │     ├── 워크플로우_차트_다이어그램.md
...
### 🌐 다국어 이미지 번역 시스템 (multilingual_text_in_image_translatio_agy_sdk_core) [2026-08-19 신설 표준]

#### 1. 핵심 아키텍처 및 파이프라인
- **엔진 본체**: `multilingual_text_in_image_translatio_agy_sdk_core/multilingual_text_in_image_translatio_agy_sdk_core.py`
- **Two-Pass 신경망 인페인팅**:
