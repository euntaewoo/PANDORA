...
- **1. 1열 라벨 너비 (Width)**:
  ...
  - **중화권 (중국어 간체 CN / 중국어 번체 TW·HK)**: `width: 270px` (Noto Sans SC/TC 26px 기준, 좌우 여백 총 30px(15px+15px), 2열 순수 본문 폭 520px 극대화 황금비율).
...
- **5. 고시정보표 전담 프론트엔드 QA 서브에이전트 (`notice_table_frontend_qa_agent`)**:
  - **역할**: 다국어 고시정보표 1차 렌더링 즉시 PNG 캡처를 `view_file`로 자동 디코딩하여 1열 라벨 270px/295px/280px 규격, 10자 미만 1줄 유지, 2열 본문 외톨이 글자(Orphan) 유무를 픽셀 단위로 전담 자가 검수 및 자동 교정 수행.

---

## 12. 다국어 이커머스 SEO/GEO/AEO 마이크로-써머리(TXT) 작성 및 광고법 준수 표준 규격
- **1. [HARD STOP] AI 부연 설명 및 글자수 카운트 메타 주석 일체 출력 금지 (Zero Meta Commentary)**:
...
- **2. 각 국가별 전자상거래 및 광고법(Ad-Law) 사전 준수 의무**:
