...
- **[타이포그래피 폰트 크기 규격]**:
  ...
  - **중국어(CN / TW / HK)**:
    - **상단 타이틀**: **`56px`** (Bold)
    - **테이블 좌측 항목명 (항목 라벨 열)**: **`28px`** (Bold)
    - **테이블 우측 본문 내용 (값 열)**: **`28px`** (Regular)
...
- **권역별 표준 규격**:
  - **중국 본토 (간체자, zh-CN)**: 간체자 출력, 중국 신(新) 광고법 절대화 표현 필터링, 타오바오/티몰 톤앤매너, 알리바바 푸후이체 간체 렌더링.
  - **대만/홍콩 (번체자, zh-TW / zh-HK)**: 번체자 출력, 대만 TFDA 규정 및 현지 이커머스 어휘(鎖水, 水光肌 등), 알리바바 푸후이체 번체 렌더링.
