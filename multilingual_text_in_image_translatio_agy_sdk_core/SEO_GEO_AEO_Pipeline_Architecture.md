...
sequenceDiagram
    ...
    participant Script as multilingual_text_in_image_translatio_agy_sdk_core.py
...
- **특징**:
  - 고객센터 번호(`+82-2-6743-3206`) 필수 포함 규정 강제 적용.
  - 템플릿에 고정된 상품명(예: 아쿠아타이드)이 아닌 파이프라인에서 현재 처리 중인 타겟 폴더의 상품명(`{product_name}`)을 실시간 동적 매핑하여 각 상품별 맞춤형 질의응답을 완벽하게 생성하도록 동기화됨.

---

## 4. 프롬프트 엔지니어링 재사용 가이드 (Maintainer Guide)
- 파이프라인 프롬프트를 수정할 경우 `multilingual_text_in_image_translatio_agy_sdk_core.py` 내의 `generate_seo_geo_aeo_txt` 함수에 선언된 `prompt_kr`, `prompt_en`, `prompt_cn`, `prompt_jp`, `prompt_tw` 변수를 직접 수정합니다.
...
- **다국어 폰트 최적화 규격 (타이포그래피 차등 적용)**: 
  - **한국어 (Pretendard)**: 타이틀 `60px`, 본문 `30px`, 1열 `width: 295px; padding: 24px 20px;`
  - **영어 (Pretendard)**: 타이틀 `60px`, 본문 `30px` (전성분 `24px`), **1열 `width: 295px; padding: 14px 12px; letter-spacing: -0.8px;` (순수 가용폭 `271px` 확보, `Country of Origin`, `Precautions for Use` 100% 1줄 안착, `word-break: keep-all; overflow-wrap: break-word;`)**
  - **일본어 (Noto Sans JP)**: 타이틀 `64px`, 본문 `32px`, 1열 `width: 280px; padding: 24px 20px;`
  - **중국어 간체/번체 (Noto Sans SC/TC)**: 글자가 뚱뚱해지는 한자(방괴자) 특성을 고려하여 다운스케일링 ➔ 타이틀 `52px`, 본문 `26px`, 1열 `width: 250px; padding: 20px 15px;` (2열 순수 본문 폭 570px)
- **1. [Smart Layout] 1열 고정폭 및 2열 오버플로우 방지 룰**
   - **1열 라벨(th)**: `table-layout: fixed; word-break: keep-all;` 로 의미 단위 줄바꿈 적용. 영문 전용 `padding: 14px 12px; letter-spacing: -0.8px;` 적용으로 `Country of Origin`, `Precautions for Use`, `Customer Service` 100% 1줄 안착 및 `Quality Assurance<br>Standard`, `Manufacturer /<br>Distributed by` 독립 샌드박스 대칭 개행 적용.
   - **2열 본문(td)**: `word-break: keep-all; overflow-wrap: break-word;` 적용. 전성분 번역 시 **국제화장품원료집(INCI) 및 한국화장품성분사전(KCID) 표준 명칭과 해당 국가 표기법**을 준수하여 전문적인 포맷팅(띄어쓰기/콤마)을 통해 자연스러운 의미 단위 줄바꿈을 유도함.
- **[핵심] 글로벌 뷰티 번역 표준 명칭 (이커머스/현지 법령 강제 매핑)**:
  - **영어 (Amazon/Sephora US)**: 
    - 용량: `Size / Net Wt.` | 피부타입: `Skin Type` | 기한: `Shelf Life / Period After Opening` | 사용법: `Directions` | 제조/판매: `Manufacturer / Distributed by` | 제조국: `Country of Origin` | 성분: `Ingredients` | 기능성: `Functional Cosmetics Review Status` | 주의사항: `Precautions for Use` | 보증: `Quality Assurance Standard` | 상담: `Customer Service`
  - **일본어 (약기법/Qoo10 Japan)**:
...
- **렌더링 엔진**: `00_공통자료/render_notice_table_standard.py` 표준 모듈 가동 후 Headless Edge 스냅샷 처리 및 여백 크롭 렌더링 수행.
