...
# 루트 폴더 이동에 따른 인증키 fallback 경로 자동 탐색
if not gcp_json_key or not os.path.exists(gcp_json_key):
    fallback_key_path = os.path.join(script_dir, "00_공통자료", "인증키_및_계정", "김차장_vertex api_key", "vertex_ai_auth_key.json")
...
if len(sys.argv) > 1:
    ...
    # 기본 경로 세팅 (개발/테스트용)
    source_dir = r"C:\Users\euntaewoo\Desktop\multilingual_text_in_image_translatio_agy_sdk\01_번역대상_원본"
    base_target_dir = r"C:\Users\euntaewoo\Desktop\multilingual_text_in_image_translatio_agy_sdk\02_번역결과_최종"
...
13. 商品情報告示(표): '기능성화장품 심사필유무'는 '区分'으로, 결과는 오직 '化粧品'으로 표기. '제조국'->'製造国', '소비자상담실'->'消費者相談室'.
